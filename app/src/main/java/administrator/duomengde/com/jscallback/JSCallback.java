package administrator.duomengde.com.jscallback;
...
import administrator.duomengde.com.api.NetConfig;
    ...
    public void gallery(String response) {
        try {
            ...
            if (mImageList != null && mImageList.size() > 0) {
                ...
                Toast.makeText(BaseApp.getAppContext(), "这个地方显示图片", Toast.LENGTH_SHORT).show();
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
...
//        playVideo(type, src, videoStream, refer);
        Toast.makeText(BaseApp.getAppContext(), "播放视频把", Toast.LENGTH_SHORT).show();
    }
...
//        playVideoByBrowser(url, target, title);
        Toast.makeText(BaseApp.getAppContext(), "打开浏览器", Toast.LENGTH_SHORT).show();
    }
    ...
    public void jumpInnerPage(final String url) {
       /* mWebContent.post(new Runnable() {
        ...
        Toast.makeText(BaseApp.getAppContext(), "打开一个什么地址", Toast.LENGTH_SHORT).show();
    }
    ...
    public void appShare(final String json) {
        ...
        Toast.makeText(BaseApp.getAppContext(), "这是个分享功能", Toast.LENGTH_SHORT).show();
    }

    @JavascriptInterface
    public void getAuthorization() {
        Toast.makeText(BaseApp.getAppContext(), "这个地方调用了js里的一个方法，和token、UUID有关", Toast.LENGTH_SHORT).show();
       /* mWebContent.post(new Runnable() {
            @Override
            public void run() {
    ...
    public void payMoney(final String json) {
        Toast.makeText(BaseApp.getAppContext(), "支付功能", Toast.LENGTH_SHORT).show();
    }
    ...
    public void shareUrl(final String url, final String title, final String desc, final String shareTitle) {
        ...
        Toast.makeText(BaseApp.getAppContext(), "fenxiang功能", Toast.LENGTH_SHORT).show();
    }

    @JavascriptInterface
    public void shareArticle(final String json) {
        Toast.makeText(BaseApp.getAppContext(), "分享文章", Toast.LENGTH_SHORT).show();
    }
    ...
    public void cancel_order(final String orderNo) {
        if (TextUtils.isEmpty(orderNo))
            return;
        Toast.makeText(BaseApp.getAppContext(), "取消命令", Toast.LENGTH_SHORT).show();
...
//        });
    ...
    public void pay(final String orderNo, final String orderPrice) {
        Toast.makeText(BaseApp.getAppContext(), "又是支付功能", Toast.LENGTH_SHORT).show();
      /*  getActivity().runOnUiThread(new Runnable() {
            @Override
            public void run() {
                if (TextUtils.isEmpty(orderNo)) {
